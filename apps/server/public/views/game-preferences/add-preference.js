...
function showSelectionPanel(game) {
	...
	addBtn.addEventListener("click", async () => {
		...
		try {
			...
			const gameIdToSend =
				game.appid != null && !Number.isNaN(Number(game.appid))
					? Number(game.appid)
					: game._id || game.appid;
			const body = {
				email: u.email,
			...
			document.getElementById("status").textContent =
				"Juego agregado a tus preferencias.";
			// refresh local user
			const fresh = await (async () => {
				const e = encodeURIComponent(u.email);
				const r = await fetch(`/api/users/email/${e}`);
...
document.addEventListener("DOMContentLoaded", async () => {
	...
	let games = [];
	try {
		games = await loadGamesList();
	} catch (e) {
		document.getElementById("status").textContent =
			"No se pudieron cargar los juegos: " + String(e);
		return;
	}
	...
	async function searchGames(term) {
		try {
			const res = await fetch(
				`/api/games/search?q=${encodeURIComponent(term)}`,
			);
			if (!res.ok) {
				document.getElementById("status").textContent =
					`Error ${res.status} searching games`;
				return [];
			}
			...
			document.getElementById("status").textContent =
				"Error searching games: " + String(err);
			return [];
		}
	}
	...
	if (searchBtn) {
		searchBtn.addEventListener("click", async () => {
			...
			if (!term || term.trim().length === 0)
				return renderResults(games.slice(0, 50));
