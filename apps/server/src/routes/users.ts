			...
			.post(
				...
				async ({ body: { userId, gameId, rating, notes }, status }) => {
					...
					try {
						if (
							gameId != null &&
							(typeof gameId === "number" || String(gameId).match(/^\d+$/))
						) {
						...
						if (!game) {
							try {
								game = await GameModel.findById(String(gameId)).exec();
							} catch (e) {
								/* ignore */
							}
						}
					...
					const updated = await User.findOneAndUpdate(
						...
						{
							$set: {
								"gamePreferences.$.rating": rating,
								"gamePreferences.$.notes": notes ?? "",
							},
						},
						{ new: true },
					).exec();
					...
					if (updated)
						return status(200, { message: "Preferencia actualizada" });
				...
				async ({ body: { userId, email, gameId }, status }) => {
					try {
						...
						if (!query._id && !query.email)
							return status(400, { message: "Se requiere userId o email" });
						const updated = await User.findOneAndUpdate(
							query,
							{ $pull: { gamePreferences: { gameId: game._id } } },
							{ new: true },
						).exec();
						if (!updated)
							return status(404, { message: "Usuario no encontrado" });
