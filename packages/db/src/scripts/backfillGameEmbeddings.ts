...
const main = async () => {
	console.log(
		"[vector-backfill] Iniciando generación de embeddings para juegos sin vector...",
	);
	...
	for await (const game of cursor) {
		processed += 1;
		const label =
			typeof game.appid === "number" ? game.appid : String(game._id);
		try {
			...
			console.error(
				`[vector-backfill] Error al generar embedding para ${label}:`,
				error,
			);
		}
		if (DELAY_MS > 0) {
		...
		if (BATCH_SIZE > 0 && processed % BATCH_SIZE === 0) {
			console.log(
				`[vector-backfill] Progreso: procesados=${processed} actualizados=${updated}`,
			);
		}
	}

	console.log(
		`[vector-backfill] Finalizado. Procesados=${processed} actualizados=${updated}`,
	);
	process.exit(0);
};
