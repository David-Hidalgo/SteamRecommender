    ...
    const pipeline: mongoose.PipelineStage[] = [
        ...
        $lookup: {
            ...
            { $match: { $expr: { $ne: ["$_id", "$$targetId"] } } },
            {
                ...
                appid: 1,
                name: 1,
                  tags: { $ifNull: ["$data.tags", { $ifNull: ["$data.genres", { $ifNull: ["$data.categories", []] }] }] },
                popularity: { $ifNull: ["$data.popularity", 1] },
                },
            },
            {
                $addFields: {
                  commonTags: { $size: { $setIntersection: [{ $ifNull: ["$tags", []] }, { $ifNull: ["$$targetTags", []] }] } },
                },
            },
        ...
        // buscar en usuarios que también tienen esos juegos en sus preferencias
            {
                $lookup: {
                    ...
                    let: { liked: "$likedIds" },
                    pipeline: [
                        { $match: { $expr: { $gt: [{ $size: { $ifNull: ["$gamePreferences", []] } }, 0] } } },
...
export async function recommendGamesByPublisher(
    ...
    const pipeline: mongoose.PipelineStage[] = [
        ...
        },
        {
            $project: {
                ...
                publisherMatched: {
                    $cond: [
                        { $regexMatch: { input: { $ifNull: ["$data.publisher", ""] }, regex: regex, options: "i" } },
                        "$data.publisher",
                        {
                            $cond: [
    ...
    try {
        ...
        return (results || []).map((r: any) => ({
            ...
            name: r.name,
            score: typeof r.score === "number" ? r.score : 0,
            reason: { publisherMatched: r.publisherMatched, popularity: r.popularity },
        }));
    } catch (error) {
...
export async function recommendByGameContext(
    ...
    const targetTags: string[] = (target.data?.tags || target.data?.genres || target.data?.categories) ?? [];
    const targetPublishers: string[] = (() => {
        if (Array.isArray(target.data?.publishers) && target.data.publishers.length) return target.data.publishers;
        if (typeof target.data?.publisher === "string") return [target.data.publisher];
        return [];
    })();
        // categories not used directly here (kept for future use)
    const franchise = target.data?.franchise || target.data?.series || extractFranchiseFromName(name);

    // Prepare queries
    const franchiseRegex = franchise ? new RegExp("^" + escapeRegex(String(franchise)), "i") : null;
    ...
    if (franchiseRegex) {
        franchisePipeline.push(
            ...
            { $match: { $or: [{ name: { $regex: franchiseRegex } }, { "data.franchise": franchise }] } },
            { $project: { appid: 1, name: 1, popularity: { $ifNull: ["$data.popularity", 1] } } },
            { $addFields: { score: { $multiply: [100, "$popularity"] }, reason: { type: "franchise", franchise: franchise } } },
            { $sort: { score: -1 } },
            { $limit: limit },
        );
    }
            ...
            $match: {
                $or: [
                    ...(targetPublishers.length
                        ? targetPublishers.map((p) => ({ "data.publishers": { $elemMatch: { $regex: p, $options: "i" } } }))
                        : []),
                    ...(targetPublishers.length ? targetPublishers.map((p) => ({ "data.publisher": { $regex: p, $options: "i" } })) : []),
                ],
            },
        },
        { $project: { appid: 1, name: 1, popularity: { $ifNull: ["$data.popularity", 1] }, publishers: "$data.publishers", publisher: "$data.publisher" } },
        { $addFields: { score: { $multiply: [50, "$popularity"] }, reason: { type: "publisher" } } },
        { $sort: { score: -1 } },
        { $limit: limit },
    ];
    ...
    const genrePipeline: mongoose.PipelineStage[] = [
        ...
        { $addFields: { commonTags: { $size: { $setIntersection: [{ $ifNull: ["$tags", []] }, tags] } } } },
        { $match: { commonTags: { $gt: 0 } } },
        { $addFields: { score: { $multiply: ["$commonTags", "$popularity"] }, reason: { type: "tags", commonTags: "$commonTags" } } },
        { $sort: { score: -1 } },
        { $limit: limit * 2 },
    ];
    ...
    const runs = await Promise.all([
        franchiseRegex ? Game.aggregate(franchisePipeline).exec() : Promise.resolve([]),
    ...
    for (const r of publisherRes || []) {
        pushResult(r, r.score ?? 50, { type: "publisher", publishers: r.publishers ?? r.publisher ?? null });
    }
...
export default { recommendSimilarGamesByContent, recommendForUserCollaborative, recommendGamesByPublisher, recommendByGameContext };
